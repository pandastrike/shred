  ...
  request._raw = http.request(reqParams, function(response) {
    ...
    response = new Response(response, request, function(response) {
      ...
      var emit = function(event) { 
        if (request.emitter.listeners(response.status).length>0 || request.emitter.listeners(HTTP.STATUS_CODES[response.status].toLowerCase()).length>0) {
          request.emitter.emit(response.status,response);
          request.emitter.emit(HTTP.STATUS_CODES[response.status].toLowerCase(),response);
        } else {
